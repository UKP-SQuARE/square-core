...
class GraphTransformers(Model):
    ...
    def convert_features_to_tensors(self, features):
        ...
        return all_input_ids, all_input_mask, \
               all_segment_ids, all_output_mask, all_label
    ...
    def _convert_examples_to_features(
        ...
        labels = ord(examples["answerKey"]) - ord("A") \
            if 'answerKey' in examples else 0
    ...
    def load_sparse_adj_data_with_contextnode(
        ...
        concept_ids = torch.full(
            (n_samples, max_node_num), 1, dtype=torch.long)
        node_type_ids = torch.full(
            (n_samples, max_node_num), 2, dtype=torch.long)  # default 2: "other node"
        node_scores = torch.zeros(
            (n_samples, max_node_num, 1), dtype=torch.float)
    ...
    def _predict(self, request: PredictionRequest) \
        ...
        *test_decoder_data, test_adj_data = self.load_sparse_adj_data_with_contextnode(
        ...
        output_lm_subgraph = request.model_kwargs.get(
            "output_lm_subgraph",
            False
        )
        topk_lm_scores = request.model_kwargs.get(
            "topk_lm_scores",
            50
        )
        output_attn_subgraph = request.model_kwargs.get(
            "output_attn_subgraph",
            False
        )
        topk_attn = request.model_kwargs.get(
            "topk_attn",
            5
        )
        ...
        with torch.no_grad():
            if not output_attn_subgraph:
                ...
                logits, attn, concept_ids, node_type_ids, \
                edge_index_orig, edge_type_orig = self.model(
                    *input_data,
                    detail=output_attn_subgraph
                )
    ...
    def _sequence_classification(
            self, request: PredictionRequest) -> PredictionOutput:
        """
