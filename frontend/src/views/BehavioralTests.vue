<template>
  <div>
    <form v-on:submit.prevent="showCheckList">
      <div class="row">
        <div class="col">
          <CompareSkills selector-target="explain" :skill-filter="skillId => skillId in checklistData"
            v-on:input="changeSelectedSkills" class="border-success" />
        </div>
      </div>
        ...
        <div class="col table-responsive bg-light border border-primary rounded shadow p-3 mx-3 mt-4">
          <table class="table table-borderless">
            <thead class="border-bottom border-dark">
              <tr>
                <th v-for="(skill, index) in currentSkillsFulfilled" :key="index" scope="col"
                  class="fs-2 fw-light text-center">{{ skill.name }}</th>
              </tr>
              <tr>
                <th v-for="index in currentSkillsFulfilled.length" :key="index" scope="col" class="fw-normal text-center">
                  <a v-on:click="downloadExamples(index)" :ref="`downloadButton${index}`"
            ...
            <tbody>
              <tr v-for="row in currentTests[0].length" :key="row">
                <td v-for="index in currentSkillsFulfilled.length" :key="index"
                  :width="`${100 / currentSkillsFulfilled.length}%`" style="min-width: 320px;">
                  <div class="progress flex-grow-1 align-self-center m-2" title="Failure rate">
                    <div class="progress-bar bg-danger" role="progressbar"
                      ...
                      aria-valuemin="0" aria-valuemax="100">{{ getTest(index, row).failed_cases }}</div>
                    <div class="progress-bar bg-success" role="progressbar"
                      ...
                      aria-valuemin="0" aria-valuemax="100">{{ getTest(index, row).success_cases }}</div>
                  </div>
                  <div class="text-center">
                    <h3 class="my-3">{{ getTest(index, row).test_name }}</h3>
                    <p class="d-inline-flex align-items-center">
                      <BadgePopover :popover-title="mapTestType(getTest(index, row).test_type)"
                        :popover-content="getTest(index, row).test_type_description" />
                      test on
                      <BadgePopover :popover-title="getTest(index, row).capability"
                        :popover-content="getTest(index, row).capability_description" />
                    </p>
                    <div>
                      <a class="btn btn-outline-secondary d-inline-flex align-items-center" data-bs-toggle="modal"
                        :data-bs-target="`#modal-${index}-${row}`" role="button">
                        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor"
    ...
    <div v-else class="row">
      <div class="col-md-8 mx-auto mt-4 text-center">
        <div class="bg-light border rounded shadow p-5 text-center">
          ...
          <h2 class="display-5">Behavioral Tests</h2>
          <p class="lead fs-2">Analyze the <span class="text-success">I/O</span> behavior of the models.</p>
