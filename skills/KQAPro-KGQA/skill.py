...
async def predict(
    ...
    # SELECT DISTINCT ?p WHERE { ?e_1 <pred:name> "Little Shop of Horrors" . ?e_2 <pred:name> "John Candy" . ?e_1 ?p ?e_2 .  }
    # model_response = {'result':{"generated_texts": ["SELECT DISTINCT ?qpv WHERE { ?e_1 <pred:name> \"Mrs. Miniver\" . ?e_2 <pred:name> \"Academy Award for Best Writing, Adapted Screenplay\" . ?e_1 <award_received> ?e_2 . [ <pred:fact_h> ?e_1 ; <pred:fact_r> <award_received> ; <pred:fact_t> ?e_2 ] <statement_is_subject_of> ?qpv .  }"]}}
...
def get_sparql_answer(sparql, rel2type="./skill/relation2type.json"):
    """
...
if __name__ == "__main__":
    ...
    sparql = model_response["generated_texts"][0]
    sparql = post_process(sparql)
    answer = get_sparql_answer(
        sparql,
        rel2type="/storage/ukp/work/fang/square/square-core/skills/KQAPro-KGQA/relation2type.json",
    )
    print(answer)
