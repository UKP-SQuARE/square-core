...
class TestOnnxInference:
    ...
    async def test_question_answering(
        self, prediction_request, test_onnx_question_answering, input
    ):
        prediction_request.input = input
        prediction_request.task_kwargs = {"topk": 1}

        prediction = test_onnx_question_answering.predict(
    ...
    async def test_question_answering_topk(
        self, prediction_request, test_onnx_question_answering
    ):
    ...
    async def test_quantized_question_answering(
        self, prediction_request, test_onnx_quantized_question_answering, input
    ):
        prediction_request.input = input
        prediction_request.task_kwargs = {"topk": 1}

        prediction = test_onnx_quantized_question_answering.predict(
            prediction_request, Task.question_answering
        )
        answers = [
            input[i][1][prediction.answers[i][0].start : prediction.answers[i][0].end]
    ...
    @pytest.mark.asyncio
    async def test_quantized_question_answering_topk(
        self, prediction_request, test_onnx_quantized_question_answering
    ):
        input = [["What is a test?", "A test is a thing where you test."]]
        prediction_request.input = input
        prediction_request.task_kwargs = {"topk": 2}
    ...
    async def test_onnx_categorical(
        self, prediction_request, test_onnx_categorical, input
    ):
        prediction_request.input = input
        prediction_request.task_kwargs = {"topk": 1}

        prediction = test_onnx_categorical.predict(
    ...
    async def test_sequence_classification(
        ...
        prediction = test_onnx_sequence_classification.predict(
            prediction_request, Task.sequence_classification
        )
    ...
    async def test_token_classification(
        ...
        prediction = test_onnx_token_classification.predict(
    ...
    async def test_embedding(
        self, prediction_request, test_onnx_embedding, input, mode
    ):
    ...
    async def test_embedding_pooler_not_available(
        self, prediction_request, test_onnx_embedding, input, mode
    ):
        if test_onnx_embedding is None:
            pytest.skip("No model found.")
    ...
    async def test_embedding_unknown_mode(
        self, prediction_request, test_onnx_embedding
    ):
        if test_onnx_embedding is None:
            pytest.skip("No model found.")
    ...
    async def test_generation_beam_sample_multiple_seqs(
        self, prediction_request, test_onnx_generation, input
    ):
        if test_onnx_generation is None:
            pytest.skip("No model found.")
        prediction_request.input = input
