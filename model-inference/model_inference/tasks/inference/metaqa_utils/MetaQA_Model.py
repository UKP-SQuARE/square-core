...
class MetaQA_Model(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.list_MoSeN = nn.ModuleList(
            [nn.Linear(config.hidden_size, 1) for i in range(self.num_agents)]
        )
        self.input_size_ans_sel = 1 + config.hidden_size
        interm_size = int(config.hidden_size / 2)
        self.ans_sel = nn.Sequential(
            nn.Linear(self.input_size_ans_sel, interm_size),
            nn.ReLU(),
