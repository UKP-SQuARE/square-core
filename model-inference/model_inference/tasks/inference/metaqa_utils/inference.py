...
class MetaQA_basemodel():
    ...
    def run_metaqa(self, request: PredictionRequest):
        ...
        # Get predictions
        (pred, agent_name, metaqa_score, agent_score) = self._get_predictions(logits.detach().numpy(),
                                                                              request.input_predictions)
    ...
    def _encode_metaQA_instance(self, request: PredictionRequest, max_len=512):
        ...
        list_input_ids.extend(
            self.tokenizer.encode(request.input_question, add_special_tokens=False))  # Query token ids
